...
BOOL CManagerDialog::OnInitDialog()
{
	...
	// 设置窗口默认大小和最小尺寸
	int initWidth = g_data.DPI(730);
	int initHeight = g_data.DPI(500);
	m_min_size.cx = g_data.DPI(660);
	m_min_size.cy = g_data.DPI(440);
	...
	// 初始化 持仓列表
	m_pos_listctrl.SetExtendedStyle(dwStyle);
	...
	// 初始化 自定义分组列表
	m_custom_listctrl.SetExtendedStyle(dwStyle);
...
void CManagerDialog::RefreshStockList()
{
	...
	for (size_t i = 0; i < m_data.m_stock_codes.size(); ++i)
	{
		...
		auto itAlert = g_data.m_stock_alert_prices.find(code);
		if (itAlert != g_data.m_stock_alert_prices.end())
		{
			if (itAlert->second.first > 0)
			{
				CString lowStr;
				lowStr.Format(_T("%.2f"), itAlert->second.first);
				m_stock_listctrl.SetItemText(nItem, 2, lowStr);
			}
			if (itAlert->second.second > 0)
			{
				CString highStr;
				highStr.Format(_T("%.2f"), itAlert->second.second);
				m_stock_listctrl.SetItemText(nItem, 3, highStr);
			}
		}

		auto itBar = g_data.m_stock_statusbar.find(code);
		if (itBar != g_data.m_stock_statusbar.end() && itBar->second)
		{
...
void CManagerDialog::RefreshPositionList()
{
	...
	{
		auto itPos = g_data.m_stock_positions.find(code);
		if (itPos != g_data.m_stock_positions.end() && (itPos->second.cost_price > 0 || itPos->second.holding_count > 0))
		{
			...
			strCost.Format(_T("%.2f"), itPos->second.cost_price);
			strCount.Format(_T("%d"), itPos->second.holding_count);
			strDate = itPos->second.buy_date.c_str();

			double totalCost = itPos->second.cost_price * itPos->second.holding_count;
			strTotal.Format(_T("%.2f"), totalCost);

			double curPrice = 0.0;
			auto stockData = g_data.GetStockData(code);
			if (stockData)
				curPrice = stockData->info.price;

			strCurPrice.Format(_T("%.2f"), curPrice);

			double profit = (curPrice - itPos->second.cost_price) * itPos->second.holding_count;
...
void CManagerDialog::UpdateControlsLayout()
{
	...
	int rightLeft = m_menu_width + g_data.DPI(20);
	int rightTop = g_data.DPI(65);
	int rightWidth = clientRect.Width() - rightLeft - g_data.DPI(20);
	...
	const int basicControlIds[] = {
		...
		IDC_KLINE_WIDTH_EDIT, IDC_KLINE_HEIGHT_EDIT
	};
	...
	if (isBasic)
	{
		int itemTop = rightTop + g_data.DPI(15);
		int lineHeight = g_data.DPI(28);
		int checkW = g_data.DPI(160);

		int checkIds[] = { IDC_FULL_DAY_CHECK, IDC_SHOW_STOCK_NAME_CHECK, IDC_COLOR_WITH_PRICE_CHECK, IDC_SHOW_FLUCTUATION_CHECK };
		for (int id : checkIds)
		{
			CWnd* pWnd = GetDlgItem(id);
			if (pWnd && pWnd->GetSafeHwnd())
				pWnd->MoveWindow(rightLeft, itemTop, checkW, g_data.DPI(20));
			itemTop += lineHeight;
		}

		itemTop += g_data.DPI(10);
		CWnd* pProxyChk = GetDlgItem(IDC_USE_SOCKS5_PROXY_CHECK);
		if (pProxyChk && pProxyChk->GetSafeHwnd())
			pProxyChk->MoveWindow(rightLeft, itemTop, g_data.DPI(140), g_data.DPI(20));

		CWnd* pProxyLbl = GetDlgItem(IDC_SOCKS5_PROXY_STATIC);
		if (pProxyLbl && pProxyLbl->GetSafeHwnd())
			pProxyLbl->MoveWindow(rightLeft + g_data.DPI(145), itemTop + g_data.DPI(2), g_data.DPI(60), g_data.DPI(18));

		CWnd* pProxyEdit = GetDlgItem(IDC_SOCKS5_PROXY_EDIT);
		if (pProxyEdit && pProxyEdit->GetSafeHwnd())
			pProxyEdit->MoveWindow(rightLeft + g_data.DPI(210), itemTop, g_data.DPI(180), g_data.DPI(22));

		itemTop += lineHeight + g_data.DPI(10);

		CWnd* pKWEdit = GetDlgItem(IDC_KLINE_WIDTH_EDIT);
		if (pKWEdit && pKWEdit->GetSafeHwnd())
			pKWEdit->MoveWindow(rightLeft + g_data.DPI(80), itemTop, g_data.DPI(70), g_data.DPI(22));

		CWnd* pKHEdit = GetDlgItem(IDC_KLINE_HEIGHT_EDIT);
		if (pKHEdit && pKHEdit->GetSafeHwnd())
			pKHEdit->MoveWindow(rightLeft + g_data.DPI(240), itemTop, g_data.DPI(70), g_data.DPI(22));
	}
	...
	if (isWebDav)
	{
		int wdTop = rightTop + g_data.DPI(10);
		int lblW = g_data.DPI(75);
		int editW = min(g_data.DPI(300), rightWidth - lblW - g_data.DPI(20));
		int rowH = g_data.DPI(26);

		// URL
		CWnd* pUrlLbl = GetDlgItem(IDC_WEBDAV_URL_STATIC);
		CWnd* pUrlEdit = GetDlgItem(IDC_WEBDAV_URL_EDIT);
		if (pUrlLbl && pUrlLbl->GetSafeHwnd()) pUrlLbl->MoveWindow(rightLeft, wdTop + g_data.DPI(3), lblW, g_data.DPI(18));
		if (pUrlEdit && pUrlEdit->GetSafeHwnd()) pUrlEdit->MoveWindow(rightLeft + lblW, wdTop, editW, g_data.DPI(22));
		wdTop += rowH;

		// Username
		CWnd* pUsrLbl = GetDlgItem(IDC_WEBDAV_USER_STATIC);
		CWnd* pUsrEdit = GetDlgItem(IDC_WEBDAV_USER_EDIT);
		if (pUsrLbl && pUsrLbl->GetSafeHwnd()) pUsrLbl->MoveWindow(rightLeft, wdTop + g_data.DPI(3), lblW, g_data.DPI(18));
		if (pUsrEdit && pUsrEdit->GetSafeHwnd()) pUsrEdit->MoveWindow(rightLeft + lblW, wdTop, editW, g_data.DPI(22));
		wdTop += rowH;

		// Password
		CWnd* pPwdLbl = GetDlgItem(IDC_WEBDAV_PWD_STATIC);
		CWnd* pPwdEdit = GetDlgItem(IDC_WEBDAV_PWD_EDIT);
		if (pPwdLbl && pPwdLbl->GetSafeHwnd()) pPwdLbl->MoveWindow(rightLeft, wdTop + g_data.DPI(3), lblW, g_data.DPI(18));
		if (pPwdEdit && pPwdEdit->GetSafeHwnd()) pPwdEdit->MoveWindow(rightLeft + lblW, wdTop, editW, g_data.DPI(22));
		wdTop += rowH;

		// Remote Dir
		CWnd* pDirLbl = GetDlgItem(IDC_WEBDAV_DIR_STATIC);
		CWnd* pDirEdit = GetDlgItem(IDC_WEBDAV_DIR_EDIT);
		if (pDirLbl && pDirLbl->GetSafeHwnd()) pDirLbl->MoveWindow(rightLeft, wdTop + g_data.DPI(3), lblW, g_data.DPI(18));
		if (pDirEdit && pDirEdit->GetSafeHwnd()) pDirEdit->MoveWindow(rightLeft + lblW, wdTop, editW, g_data.DPI(22));
		wdTop += rowH + g_data.DPI(6);

		// Checkboxes
		CWnd* pSyncChk = GetDlgItem(IDC_WEBDAV_AUTO_SYNC_CHECK);
		CWnd* pBakChk = GetDlgItem(IDC_WEBDAV_AUTO_BACKUP_CHECK);
		if (pSyncChk && pSyncChk->GetSafeHwnd()) pSyncChk->MoveWindow(rightLeft, wdTop, g_data.DPI(240), g_data.DPI(20));
		wdTop += g_data.DPI(24);
		if (pBakChk && pBakChk->GetSafeHwnd()) pBakChk->MoveWindow(rightLeft, wdTop, g_data.DPI(240), g_data.DPI(20));
		wdTop += g_data.DPI(30);
		...
		if (pTestBtn && pTestBtn->GetSafeHwnd()) pTestBtn->MoveWindow(rightLeft, wdTop, wdBtnW, wdBtnH);
		if (pUpBtn && pUpBtn->GetSafeHwnd()) pUpBtn->MoveWindow(rightLeft + wdBtnW + wdGap, wdTop, wdBtnW + g_data.DPI(15), wdBtnH);
		if (pDownBtn && pDownBtn->GetSafeHwnd()) pDownBtn->MoveWindow(rightLeft + wdBtnW * 2 + wdGap * 2 + g_data.DPI(15), wdTop, wdBtnW + g_data.DPI(15), wdBtnH);
	}
...
void CManagerDialog::OnPaint()
{
	...
	// 背景填充
	Gdiplus::SolidBrush bgBrush(Gdiplus::Color(255, 15, 23, 42)); // Slate 900
	...
	CRect contentRect(m_menu_width + g_data.DPI(15), g_data.DPI(65), clientRect.Width() - g_data.DPI(15), clientRect.Height() - g_data.DPI(50));
...
{
	Gdiplus::SolidBrush sideBrush(Gdiplus::Color(255, 24, 27, 36));
	g.FillRectangle(&sideBrush, 0, 0, m_menu_width, clientRect.Height());

	Gdiplus::Pen divPen(Gdiplus::Color(255, 39, 45, 60), 1.0f);
	g.DrawLine(&divPen, m_menu_width, 0, m_menu_width, clientRect.Height());

	Gdiplus::SolidBrush dotBrush(Gdiplus::Color(255, 124, 58, 237));
	...
	for (int i = 0; i < menuCount; ++i)
	{
		...
		if (i == m_current_page)
		{
			Gdiplus::SolidBrush activeBg(Gdiplus::Color(255, 124, 58, 237));
			g.FillRectangle(&activeBg, rf);

			Gdiplus::SolidBrush barBrush(Gdiplus::Color(255, 192, 132, 252));
		...
		else
		{
			if (i == m_hover_menu)
			{
				Gdiplus::SolidBrush hoverBg(Gdiplus::Color(255, 39, 45, 60));
				g.FillRectangle(&hoverBg, rf);
			}

			Gdiplus::SolidBrush txtBrush(i == m_hover_menu ? Gdiplus::Color(255, 226, 232, 240) : Gdiplus::Color(255, 148, 163, 184));
			Gdiplus::RectF textRf(static_cast<Gdiplus::REAL>(r.left + g_data.DPI(14)), static_cast<Gdiplus::REAL>(r.top), static_cast<Gdiplus::REAL>(r.Width() - g_data.DPI(14)), static_cast<Gdiplus::REAL>(r.Height()));
...
void CManagerDialog::DrawHeader(Gdiplus::Graphics& g, const CRect& clientRect)
{
	...
	g.DrawLine(&divPen, rightLeft, g_data.DPI(52), clientRect.Width() - g_data.DPI(20), g_data.DPI(52));
}
...
void CManagerDialog::DrawIndexPage(Gdiplus::Graphics& g, const CRect& contentRect)
{
	...
	for (size_t i = 0; i < presets.size(); ++i)
	{
		...
		if (isSelected)
		{
			Gdiplus::SolidBrush selBg(Gdiplus::Color(255, 124, 58, 237));
			g.FillRectangle(&selBg, rf);

			Gdiplus::Pen borderPen(Gdiplus::Color(255, 168, 85, 247), 1.0f);
			g.DrawRectangle(&borderPen, rf);

			if (rank <= 5)
			{
				Gdiplus::SolidBrush rankBg(Gdiplus::Color(255, 192, 132, 252));
				g.FillEllipse(&rankBg, x + cardW - g_data.DPI(22), y + g_data.DPI(8), g_data.DPI(15), g_data.DPI(15));

				Gdiplus::SolidBrush rankTxtBrush(Gdiplus::Color(255, 76, 29, 149));
			...
			Gdiplus::SolidBrush codeBrush(Gdiplus::Color(255, 221, 214, 254));
...
{
	// 绘制 WebDAV 状态与说明卡片
	int cardTop = contentRect.top + g_data.DPI(185);
	int cardH = contentRect.bottom - cardTop;
	if (cardH < g_data.DPI(70))
		cardH = g_data.DPI(70);

	Gdiplus::RectF panelRf(static_cast<Gdiplus::REAL>(contentRect.left), static_cast<Gdiplus::REAL>(cardTop), static_cast<Gdiplus::REAL>(contentRect.Width()), static_cast<Gdiplus::REAL>(cardH));
	Gdiplus::SolidBrush panelBg(Gdiplus::Color(255, 30, 41, 59));
	g.FillRectangle(&panelBg, panelRf);
	Gdiplus::Pen panelPen(Gdiplus::Color(255, 51, 65, 85), 1.0f);
	g.DrawRectangle(&panelPen, panelRf);
	...
	int textX = contentRect.left + g_data.DPI(14);
	int textY = cardTop + g_data.DPI(10);

	g.DrawString(L"💡 说明：采用 Windows 原生 WinINet 网络组件，零外部依赖，安全高效。", -1, &tipFont, Gdiplus::PointF(static_cast<Gdiplus::REAL>(textX), static_cast<Gdiplus::REAL>(textY)), &tipBrush);
	textY += g_data.DPI(18);
	g.DrawString(L"• 兼容坚果云（Jianguoyun）、Nextcloud/ownCloud、Alist、群晖NAS 等标准 WebDAV 网盘。", -1, &tipFont, Gdiplus::PointF(static_cast<Gdiplus::REAL>(textX), static_cast<Gdiplus::REAL>(textY)), &tipBrush);
...
void CManagerDialog::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	{
		ShellExecute(nullptr, L"open", L"https://github.com/zhongyang219/TrafficMonitorPlugins", nullptr, nullptr, SW_SHFORCE_RES_OK);
		return;
	}
...
void CManagerDialog::OnDelBtnClick()
{
	...
	else if (m_current_group_tab == TAB_POSITIONS)
	{
		...
		if (curSel >= 0)
		{
			...
			auto it = g_data.m_stock_positions.find(code);
			if (it != g_data.m_stock_positions.end())
			{
				it->second.cost_price = 0.0;
				it->second.holding_count = 0;
			}
			RefreshPositionList();
		}
	}
